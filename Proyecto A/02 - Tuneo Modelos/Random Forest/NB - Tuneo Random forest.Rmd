...
```{r}
ridge_grid <- expand.grid(mtry = c(2),
                          splitrule = "gini",
                          min.node.size = c(1,3,5,7,13,20,30))
...
ggplot(plot2, aes(x=mtry, y = ROC,color=as.factor(min.node.size)))+
  geom_point(size=3,position=position_jitter(h=0, w=3))+
  #geom_smooth()+
  geom_line()+
  #expand_limits(y = c(0,1))+
...
ggplot(plot2, aes(x=min.node.size, y = ROC,shape=as.factor(mtry)))+
  geom_point(size=3,position=position_jitter(h=0, w=3))+
  #geom_smooth()+
  #geom_line()+
  #expand_limits(y = c(0,1))+
