...
```{r}
ridge_grid <- expand.grid(mtry = c(15),
                          splitrule="gini",
                          min.node.size = c(1))
...
```{r}
depth<-c(15,20,30,35,50,70,90,110,130)
```
...
plot2<-aux %>%
  filter(
         #,ROC>0.933
         min.node.size %in% c(1,5,20,30),
         #,mtry %in% c(1,2,5,50,150)
         mtry %in% c(1,2,5,20,70,150)
         )
ggplot(plot2, aes(x=max.depth, y = ROC,shape=as.factor(mtry),color=as.factor(min.node.size)))+
  ...
  labs(title = "Resultados Métrica ROC",
       subtitle = "Resultados de la métrica ROC para el modelo Random Forests con 5-fold Cross Validation",
       x = "Profundidad del Árbol",
       color = "Tamaño minimo del ultimo nodo",
       shape = "Cantidad de variables")+
  theme_bw()
...
plot2<-aux %>%
  filter(
         max.depth %in% c(10,20,30,50,110,130)
...
ggplot(plot2, aes(x=mtry, y = ROC,
  ...
  labs(title = "Resultados Métrica ROC",
       subtitle = "Resultados de la métrica ROC para el modelo Random Forests con 5-fold Cross Validation",
       x = "Cantidad de variables",
       #shape = "Número de Árboles",
       color = "Profundidad del Árbol")+
  theme_bw()
```
...
plot2<-aux %>%
  filter(
         mtry %in% c(1,20,70,150),
         max.depth %in% c(10,30,35,50,90,130)
         )
ggplot(plot2, aes(x=min.node.size, y = ROC,
  ...
  #expand_limits(y = c(0,1))+
    labs(title = "Resultados Métrica ROC",
       subtitle = "Resultados de la métrica ROC para el modelo Random Forests con 5-fold Cross Validation",
       x = "Tamaño minimo del ultimo nodo",
       shape = "Profundidad máxima",
       color = "Cantidad de variables")+
  theme_bw()
```
...
```{r}
ridge_grid <- expand.grid(mtry = c(10),
                          splitrule="gini",
                          min.node.size = c(1))
...
ridge
write_csv(rf_resultados,"Resultados_RF.csv")
saveRDS(ridge, "RF_best_model.rds")
test["predicted"]=aux
test <- subset(test, select=c(224,1:223))
aux <- predict(ridge, test)
confusionMatrix(aux, test$phishing)
library(MLeval)
test1 <- evalm(data.frame(aux, test$phishing))
```
