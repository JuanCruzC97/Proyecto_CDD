...
set.seed(35)
k = 5
CV_folds <- createFolds(train2$phishing, k = k, list = TRUE)
...
```{r}
C_range =    c(0.01,0.011,0.012,0.013,0.014,0.015)
sigma_range =  c(0.005, 0.001,0.0025)

SVC_grid <- expand.grid(C= C_range)
...
plot1 <- ggplot(df_results, aes(x=log(C), y=ROC, color=factor(sigma)))+
geom_point(size=3, position=position_jitter(h=0,w=3))+
  theme_bw()

plot1
plot2 <- ggplot(df_results, aes(x=log(C), y=Sens, color=factor(sigma)))+
geom_point(size=3, position=position_jitter(h=0,w=3))+
  theme_bw()+
  theme(legend.position="none")
plot3 <- ggplot(df_results, aes(x=log(C), y=Spec, color=factor(sigma)))+
geom_point(size=3, position=position_jitter(h=0,w=3))+
  theme_bw()+
  theme(legend.position="none")
